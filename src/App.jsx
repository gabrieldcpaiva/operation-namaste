...
const Navbar = ({ onProductsClick }) => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ScrollTrigger.create({
        ...
        toggleClass: { className: 'bg-background/90 backdrop-blur-xl text-primary border-primary/10 drop-shadow-sm', targets: navRef.current }
      });
      ScrollTrigger.create({
        start: 'top -50',
        end: 99999,
        onEnter: () => gsap.to(navRef.current, { color: '#2E4036', duration: 0.3 }),
        onLeaveBack: () => gsap.to(navRef.current, { color: '#F2F0E9', backgroundColor: 'transparent', duration: 0.3 })
      });
    }, navRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <nav ref={navRef} className="fixed top-6 left-1/2 -translate-x-1/2 z-40 flex items-center justify-between px-6 py-4 rounded-full border border-white/10 bg-black/40 backdrop-blur-xl w-[95%] max-w-5xl transition-all duration-300 text-white">
      <div className="flex items-center gap-2">
        <img src="/images/Transparent_Logo.png" alt="Namastê" className="w-24 h-auto object-contain drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]" />
      </div>
      <div className="hidden md:flex gap-8 font-bold tracking-tight text-sm uppercase">
        <a href="#filosofia" className="hover:text-accent transition-colors">Filosofia</a>
        <a href="#pilares" className="hover:text-accent transition-colors">Pilares</a>
        <button onClick={onProductsClick} className="hover:text-accent transition-colors uppercase font-bold">Produtos</button>
        <a href="#mentoria" className="hover:text-accent transition-colors">Mentoria</a>
      </div>
...
const Hero = () => {
  ...
  return (
    <section className="relative h-[100dvh] min-h-[700px] w-full flex items-end pb-24 overflow-hidden bg-primary">
      ...
      <div ref={textRef} className="relative z-10 container mx-auto px-6 lg:px-12 max-w-7xl">
        ...
        <div className="hero-part flex flex-col sm:flex-row gap-6 items-start sm:items-center">
          ...
          <p className="text-background/80 font-medium max-w-xs text-sm border-l border-accent/40 pl-4 py-1">Redução de danos, loja premium e mentoria B2B especializada.</p>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section id="pilares" className="py-24 bg-background relative z-20 rounded-t-[3rem] -mt-8">
      <div className="container mx-auto px-4 max-w-6xl relative">
        ...
        <div ref={cardsRef} className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 pt-4 pb-12">
          
          {/* Card 1 */}
          <div className="feature-card h-full">
            <div className="bg-white h-full p-8 md:p-12 rounded-3xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] border border-black/5 hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] hover:-translate-y-2 transition-all duration-500">
              <div className="flex flex-col md:flex-row md:items-start gap-6 md:gap-8">
                <div className="w-14 h-14 md:w-16 md:h-16 bg-primary/10 rounded-2xl flex items-center justify-center text-primary shrink-0">
                  <CigaretteOff className="w-7 h-7 md:w-8 md:h-8" />
                </div>
                <div>
                  <p className="font-data text-accent text-[10px] sm:text-xs tracking-widest mb-1 sm:mb-2 uppercase">Pilar 01</p>
                  <h3 className="text-2xl sm:text-3xl font-bold mb-2 sm:mb-3 text-primary tracking-tight">Redução de Danos</h3>
                  <p className="text-dark/60 font-medium leading-relaxed text-base sm:text-lg">Curadoria premium de materiais. Acreditamos no uso consciente, garantindo a melhor experiência com mínimos riscos.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Card 2 */}
          <div className="feature-card h-full pt-0 lg:pt-12">
            <div className="bg-dark text-background h-full p-8 md:p-12 rounded-3xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.2)] border border-white/5 hover:-translate-y-2 transition-all duration-500 overflow-hidden relative">
               <img src="/images/Enhanced_Image6.png" className="absolute top-0 right-0 w-full md:w-auto h-full object-cover opacity-10 mix-blend-screen pointer-events-none" alt="" />
              <div className="flex flex-col md:flex-row md:items-start gap-6 md:gap-8 relative z-10">
                <div className="w-14 h-14 md:w-16 md:h-16 bg-white/10 rounded-2xl flex items-center justify-center text-accent shrink-0">
                  <Leaf className="w-7 h-7 md:w-8 md:h-8" />
                </div>
                <div>
                   <p className="font-data text-sage text-[10px] sm:text-xs tracking-widest mb-1 sm:mb-2 uppercase flex items-center gap-2">Pilar 02 <span className="w-2 h-2 rounded-full bg-accent animate-pulse inline-block" /> LIVE</p>
                  <h3 className="text-2xl sm:text-3xl font-bold mb-2 sm:mb-3 text-white tracking-tight">Grow Shop & Lifestyle</h3>
                  <p className="text-white/60 font-medium leading-relaxed text-base sm:text-lg">
          ...
          {/* Card 3 */}
          <div className="feature-card h-full">
            <div className="bg-primary text-background h-full p-8 md:p-12 rounded-3xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)] border border-transparent hover:-translate-y-2 transition-all duration-500 flex flex-col relative overflow-hidden">
              <div className="flex flex-col md:flex-row md:items-start gap-6 md:gap-8 relative z-10">
                <div className="w-14 h-14 md:w-16 md:h-16 bg-white/10 rounded-2xl flex items-center justify-center text-white shrink-0">
                  <BookOpen className="w-7 h-7 md:w-8 md:h-8" />
                </div>
                <div>
                  <p className="font-data text-accent text-[10px] sm:text-xs tracking-widest mb-1 sm:mb-2 uppercase">Pilar 03</p>
                  <h3 className="text-2xl sm:text-3xl font-bold mb-2 sm:mb-3 text-white tracking-tight">Mentoria Lojista B2B</h3>
          ...
          {/* Card 4 */}
          <div className="feature-card h-full pt-0 lg:pt-12">
            <div className="bg-accent text-white h-full p-8 md:p-12 rounded-3xl shadow-[0_30px_60px_-15px_rgba(204,88,51,0.4)] border border-transparent hover:-translate-y-2 transition-all duration-500 overflow-hidden relative">
              <div className="absolute -right-10 -bottom-10 md:-right-20 md:-bottom-20 opacity-20 pointer-events-none">
                <Diamond className="w-64 h-64 md:w-96 md:h-96" />
              </div>
              <div className="flex flex-col md:flex-row md:items-start gap-6 md:gap-8 relative z-10">
                <div className="w-14 h-14 md:w-16 md:h-16 bg-white/20 rounded-2xl flex items-center justify-center text-white shrink-0">
                  <Diamond className="w-7 h-7 md:w-8 md:h-8" />
                </div>
                <div>
                  <p className="font-data text-white/60 text-[10px] sm:text-xs tracking-widest mb-1 sm:mb-2 uppercase">Pilar 04</p>
                  <h3 className="text-2xl sm:text-3xl font-bold mb-2 sm:mb-3 tracking-tight">Qualidade Premium</h3>
